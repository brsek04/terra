    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 px-5 py-4">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Crear menú</h3>
            ...
            <!-- Modal Body -->
            <form action="{{ route('menus.store') }}" method="POST" enctype="multipart/form-data" class="p-2 md:p-5">
                {{ csrf_field() }}
                <div class="grid gap-4 mb-4 grid-cols-1">
                    ...
                    <div>
                        ...
                        {{ Form::text('branch_id', null, ['class' => 'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500' . ($errors->has('branch_id') ? ' is-invalid' : ''), 'placeholder' => 'Sucursal']) }}
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($dishes as $dish)
                                <div class="flex items-center">
                                    <input id="dish{{ $dish->id }}" class="form-check-input h-4 w-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500" type="checkbox" name="dishes[]" value="{{ $dish->id }}">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($beverages as $beverage)
                                <div class="flex items-center">
                                    <input id="beverage{{ $beverage->id }}" class="form-check-input h-4 w-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500" type="checkbox" name="beverages[]" value="{{ $beverage->id }}">
